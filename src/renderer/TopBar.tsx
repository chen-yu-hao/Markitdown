...
interface Props { zh: boolean; title: string; settings: Settings; hasDocument: boolean; sourceMode: boolean; focus: boolean; typewriter: boolean; canBack: boolean; canForward: boolean; command(name: string): void; update(patch: Partial<Settings>): void; reviewEnabled?: boolean; reviewCount?: number }
...
export default function TopBar(p: Props) {
  ...
  const menus: Record<string, Item[]> = {
    ...
    edit: [item('偏好设置…', 'Preferences…', 'settings', 'Ctrl+,', false), separator, item('撤销', 'Undo', 'undo', 'Ctrl+Z'), item('重做', 'Redo', 'redo', 'Ctrl+Y'), separator, item('剪切', 'Cut', 'cut', 'Ctrl+X'), item('复制', 'Copy', 'copy', 'Ctrl+C'), item('粘贴', 'Paste', 'paste', 'Ctrl+V'), item('全选', 'Select all', 'selectAll', 'Ctrl+A'), separator,
      ...
      { label: t('段落', 'Paragraph'), disabled: !p.hasDocument, children: [
        item('正文', 'Paragraph', 'paragraph', 'Ctrl+0'), ...[1,2,3,4,5,6].map(n => item(`标题 ${n}`, `Heading ${n}`, `heading${n}`, `Ctrl+${n}`)),
        item('提升标题', 'Raise heading', 'raiseHeading', 'Ctrl+='), item('降低标题', 'Lower heading', 'lowerHeading', 'Ctrl+-'), separator,
        item('表格', 'Table', 'table', 'Ctrl+Shift+T'), item('代码块', 'Code block', 'codeblock', 'Ctrl+Shift+K'), item('插入公式块', 'Display equation', 'math', 'Ctrl+Shift+M'), item('引用', 'Quote', 'quote', 'Ctrl+Shift+Q'), item('提示块', 'Alert', 'alert:NOTE', 'Ctrl+Shift+A'), item('行内注释', 'Inline comment', 'comment'), item('有序列表', 'Numbered list', 'orderedList', 'Ctrl+Shift+['), item('无序列表', 'Bullet list', 'unorderedList', 'Ctrl+Shift+]'), item('任务列表', 'Task list', 'task', 'Ctrl+Shift+X'), separator,
        item('上插段落', 'Insert paragraph above', 'paragraphAbove', 'Ctrl+Shift+Enter'), item('下插段落', 'Insert paragraph below', 'paragraphBelow', 'Ctrl+Enter'), item('脚注', 'Footnote', 'footnote'), item('水平分割线', 'Horizontal rule', 'hr', 'Ctrl+Shift+H'), item('目录', 'Table of contents', 'toc'), item('文档元数据', 'Document metadata', 'metadata') ] },
      { label: t('审阅', 'Review'), disabled: !p.hasDocument, children: [ { label: t('启动审阅模式', 'Enable review mode'), command: 'reviewToggle', checked: p.reviewEnabled }, { label: t('上一个修改', 'Previous change'), command: 'reviewPrevious', disabled: !p.reviewCount }, { label: t('下一个修改', 'Next change'), command: 'reviewNext', disabled: !p.reviewCount }, separator, { label: t('接受当前修改', 'Accept current change'), command: 'reviewAccept', disabled: !p.reviewCount }, { label: t('接受全部修改', 'Accept all changes'), command: 'reviewAcceptAll', disabled: !p.reviewCount } ] },
      ...
      { label: t('插入节点', 'Insert node'), disabled: !p.hasDocument, children: [item('文档元数据', 'Document metadata', 'metadata'), item('正文目录', 'Document contents', 'toc'), item('表格', 'Table', 'table'), item('脚注', 'Footnote', 'footnote'), item('注释', 'Comment', 'comment'), item('下划线', 'Underline', 'underline'), separator, ...[['flowchart','流程图','Flowchart'],['sequence','时序图','Sequence diagram'],['class','类图','Class diagram'],['state','状态图','State diagram'],['pie','饼图','Pie chart'],['gantt','甘特图','Gantt chart'],['er','ER 图','ER diagram']].map(([id,cn,en])=>item(cn,en,`diagram:${id}`)), separator, ...['NOTE','TIP','IMPORTANT','WARNING','CAUTION'].map(kind=>item(`提示块 ${kind}`,`Alert ${kind}`,`alert:${kind}`))] },
    ...
    view: [ { ...item('侧边栏', 'Sidebar', 'sidebar', '', false) }, { ...item('源代码模式', 'Source code mode', 'mode', 'Ctrl+/'), checked: p.sourceMode }, { label: t('论文版式', 'Article layout'), disabled: !p.hasDocument, children: [{ label: t('单栏', 'Single column'), checked: p.settings.readingLayout !== 'double', action: () => p.update({ readingLayout: 'single' }) }, { label: t('双栏（Nature 风格）', 'Two columns (Nature style)'), checked: p.settings.readingLayout === 'double', action: () => { p.update({ readingLayout: 'double' }); if (p.sourceMode) p.command('mode'); } }] }, { ...item('专注模式', 'Focus mode', 'focusMode', 'F8', false), checked: p.focus }, { ...item('打字机模式', 'Typewriter mode', 'typewriter', 'F9', false), checked: p.typewriter }, separator, { label: t('显示行号', 'Show line numbers'), checked: p.settings.showLineNumbers, action: () => { const enabled = !p.settings.showLineNumbers; p.update({ showLineNumbers: enabled }); if (enabled && p.hasDocument && p.settings.readingLayout === 'double' && !p.sourceMode) p.command('mode'); } }, { label: t('状态栏', 'Status bar'), checked: p.settings.showStatusBar, action: () => p.update({ showStatusBar: !p.settings.showStatusBar }) }, { label: t('格式工具栏', 'Formatting toolbar'), checked: p.settings.showToolbar, action: () => p.update({ showToolbar: !p.settings.showToolbar }) }, separator, item('放大', 'Zoom in', 'zoomIn', 'Ctrl++', false), item('缩小', 'Zoom out', 'zoomOut', 'Ctrl+-', false), item('实际大小', 'Actual size', 'zoomReset', 'Ctrl+0', false) ],
    ...
    help: [item('Markdown 指南', 'Markdown guide', 'guide', '', false), item('关于 Markit', 'About Markit', 'about', '', false)],
  };
