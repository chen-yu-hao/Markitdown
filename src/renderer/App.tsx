...
export default function App() {
  ...
  return <div className={`app ${focusMode ? 'focus-mode' : ''}`} data-testid="app" onDragOver={event => { if ([...event.dataTransfer.types].includes('Files')) event.preventDefault(); }} onDrop={event => {
    ...
    {settingsOpen && <Preferences settings={settings} zh={zh} version={version} pandoc={pandoc} update={updateSettings} documentPrefix={active?.mathNumberingPrefix ?? settings.mathNumberingPrefix} updateDocumentPrefix={value => { const doc = docsRef.current.find(item => item.id === activeIdRef.current); if (doc) change(doc.id, { ...patchOf(doc), mathNumberingPrefix: value }); }} close={closeSettings} error={showError} disableWritingModes={() => { setFocusMode(false); setTypewriter(false); }} initialCategory={settingsCategory} initialQuery={settingsQuery} />}
