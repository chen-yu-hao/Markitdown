...
function AboutContent({ zh, version, platform, arch }: { zh: boolean; version: string; platform: RuntimePlatform; arch: string }) {
  ...
  return <div className="about-content">
    ...
    <p className="about-version">Markit {version} · {platformName} {arch}</p>
...
function MarkdownGuide({ zh }: { zh: boolean }) {
  ...
  return <div className="guide-content">
    <p>{t('Markdown 指南展示常用语法和 Markit 的基本编辑方式。指南内容不会创建或修改文稿。', 'This guide covers common Markdown syntax and the basic Markit workflow. It does not create or modify a document.')}</p>
    <section>
...
export default function App() {
  ...
  const paperEditor = useRef<ArticlePreviewHandle>(null);
  ...
  const select = useCallback((id: string, record = true) => {
    if (activeIdRef.current !== id) editors.current.get(activeIdRef.current)?.flushScroll();
  ...
  useEffect(() => { document.title = active ? `${active.dirty || active.recovered ? '* ' : ''}${active.title} - Markit` : 'Markit'; }, [active?.title, active?.dirty, active?.recovered]);
  useEffect(() => {
  ...
  const change = useCallback((id: string, patch: DocumentPatch) => {
    ...
    else if (patch.scrollTop !== doc.scrollTop) void window.markedown.updateScroll(id, patch.scrollTop, patch.editVersion).catch(error => showError(String(error)));
  }, [replaceDocs, showError]);
  const scroll = useCallback((id: string, scrollTop: number, editVersion: number) => {
    const doc = docsRef.current.find(item => item.id === id);
    if (!doc || doc.editVersion !== editVersion || doc.scrollTop === scrollTop) return;
    docsRef.current = docsRef.current.map(item => item.id === id ? { ...item, scrollTop } : item);
    // The Editor coalesces wheel events per frame. Send only the position;
    // neither React nor the file/autosave path needs a full document update.
    void window.markedown.updateScroll(id, scrollTop, editVersion).catch(error => {
      if (!removedDocuments.current.has(id)) showError(String(error));
    });
  }, [showError]);
  ...
  async function save(id = activeIdRef.current, saveAs = false) {
    editors.current.get(id)?.flushScroll();
  ...
  async function closeDocument(id: string, others = false) {
    ...
    for (const editor of editors.current.values()) editor.flushScroll();
  ...
  actions.current = name => {
    if (tabBusyRef.current) return;
    if (nodeEditorCommand(name)) return;
    if (!['save', 'saveAs', 'zoomIn', 'zoomOut', 'zoomReset'].includes(name)) closeNodeEditor();
    ...
    if (paperPreview && paperEditor.current?.command(name)) return;
  ...
  function goToHeading(id: string) {
    if (paperPreview) {
      paperEditor.current?.scrollToHeading(id);
      return;
    }
  ...
  // reconfigured CodeMirror and rebuilt rendered tables unnecessarily.
  const documentSettingsKey = documents.map(doc => `${doc.id}:${doc.mathNumberingPrefix ?? settings.mathNumberingPrefix}`).join('\u0001');
  const editorSettings = useMemo(() => new Map(documents.map(doc => {
    const prefix = doc.mathNumberingPrefix ?? settings.mathNumberingPrefix;
    return [doc.id, prefix === settings.mathNumberingPrefix ? settings : { ...settings, mathNumberingPrefix: prefix }] as const;
  ...
  return <div className={`app ${focusMode ? 'focus-mode' : ''}`} data-testid="app" onDragOver={event => { if ([...event.dataTransfer.types].includes('Files')) event.preventDefault(); }} onDrop={event => {
    ...
    {aboutOpen && <Modal title={t('关于 Markit', 'About Markit')} onClose={closeAbout}><AboutContent zh={zh} version={version} platform={platform} arch={arch} /></Modal>}
