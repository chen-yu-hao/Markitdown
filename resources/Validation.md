...
日期：2026-09-14，Windows 11 x64、Node.js 24.14.0、Electron 44.2.0。

- TypeScript 检查与生产构建通过；35 个测试文件、342 项单元测试通过，1 项 Windows 符号链接权限测试跳过。
- 实际 Electron 验证点击段落后保存、即时排版与源码中的 Ctrl+S / Ctrl+C、不同选区，以及保存期间继续输入和滚动，共 17 组检查；保存前后文稿高度、DOM 和滚动位置稳定，新输入仍保留未保存状态。
- 长图在 1280/760 像素窗口中按正文宽度显示、纵横比保持不变，取消 600/260 像素显示高度限制；上下滚动保持响应。图片服务测试覆盖 3000×6000 图片生成 2048×4096 缓存预览。
- 双栏实际 Electron 与打包 EXE 检查通过：菜单切换、未保存内容、公式、本地图片、多标签隔离、窄窗口、Ctrl+/ 返回源码、跨模式撤销/重做，以及 HTML/PDF/PNG 导出。已检查双栏截图、PDF 页图和导出 PNG。
- 打包 EXE 的常规验证通过：ASAR、隔离数据目录、原生 Sharp 模块、未保存内容导出，以及 100%/125%/150%/200% 显示缩放。
- 发布工作流增加上述保存、长图、双栏专项检查。双栏为阅读与内建导出版式；Pandoc 导出保持原有版式。尚未进行人工中文输入法复验。
