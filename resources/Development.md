...
可下载 GitHub 按版本标签生成的 [Source code (zip)](https://github.com/chen-yu-hao/Markitdown/archive/refs/tags/v0.4.0.zip)，或克隆仓库：
...
Windows 安装版数据通常位于 `%APPDATA%\Markit`；Linux 通常位于 `~/.config/Markit`，或 `XDG_CONFIG_HOME` 指定的位置；Windows 便携版位于程序旁的 `data`。`MARKEDOWN_DATA_DIR` 可为开发或验证指定独立目录，恢复记录可能包含完整未保存文稿。升级安装继续使用已有的 `Markedown` 数据目录；新目录已有数据时优先保留新目录，不搬移或合并恢复记录。Windows 的安装标识保持不变，以支持覆盖升级。
