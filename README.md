...
**视图 → 审阅模式** 使用文稿所在 Git 工作区的真实 `HEAD` 作为基线。审阅期间的新增内容显示为红色下划线，删除内容显示为删除线；工具栏和 **编辑 → 审阅** 菜单可逐条查看、接受当前变更，或一次接受全部变更。已跟踪文件不会创建第二个仓库；未跟踪文件会在保存后创建 `Markit review: add … as baseline` 提交。文稿不在 Git 工作区时，Markit 会先请求确认，才会在当前文件夹执行 `git init`。接受操作会把变更提交回当前项目，应用数据只保存工作区、相对路径和基线提交元数据。

审阅模式也会记录由其他工具（包括 Codex 或外部 Git 操作）写入的变更。若分支发生新的提交、rebase 或 merge，Markit 会检测到 `HEAD` 改变并静默更新基线；当前编辑缓冲区仍用于显示未提交差异。新粘贴且被文稿引用的本地图片会随相关审阅提交一起处理。
